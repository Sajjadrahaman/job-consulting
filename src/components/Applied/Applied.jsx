...
const Applied = () => {
    ...
    const filterJobs = filterOption ? data.filter((job) => job.remote_or_onsite === filterOption): data;
    ...
    return (
        <div>
            ...
            <div className='mt-10 flex  flex-row-reverse '>
                <select className='select text-white font-bold bg-violet-500'
            ...
            {
                filterJobs.map(data => <SingleApplied data={data}
                    key={data.id}
                ></SingleApplied>)
            }
        </div>
